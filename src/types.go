...
func (iperf *Iperf) makeCommand() string{
	...
	if iperf.Traffic == "udp" && iperf.Direction == "uplink" {
		return fmt.Sprintf("iperf3 -u -c %s -p %d -i %v -t %v -b %s -l %s --forceflush", iperf.Server, iperf.Port, iperf.Interval, iperf.Duration, iperf.Bitrate, iperf.BufferLen)
	} else if iperf.Traffic == "udp" && iperf.Direction == "downlink"{ // "tcp"
		return fmt.Sprintf("iperf3 -u -R -c %s -p %d -i %v -t %v -b %s -l %s --forceflush", iperf.Server, iperf.Port, iperf.Interval, iperf.Duration, iperf.Bitrate, iperf.BufferLen)
	} else if iperf.Traffic == "tcp" && iperf.Direction == "uplink" {
		return fmt.Sprintf("iperf3 -c %s -p %d -i %v -t %v -P %d -w %s --forceflush", iperf.Server, iperf.Port, iperf.Interval, iperf.Duration, iperf.Parallel, iperf.Window)
	} else if iperf.Traffic == "tcp" && iperf.Direction == "downlink" {
		return fmt.Sprintf("iperf3 -R -c %s -p %d -i %v -t %v -P %d -w %s --forceflush", iperf.Server, iperf.Port, iperf.Interval, iperf.Duration, iperf.Parallel, iperf.Window)
	} else {
